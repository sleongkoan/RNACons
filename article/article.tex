...
\subsection*{Simple Tree Distance for Secondary Structure}

\paragraph{Base Pair Tree Representation}
% base pair tree representation vs abstract shapes
% upsides and downsides of the
% representation compared to abstract shape
RNA secondary structure can represented by base pair trees, where only base pairs are considered. This representation is very similar to level 3 RNA abstract shape \cite{abstract_shapes}, albeit the stem length information is conserved.
Although this representation greatly increases the size of the auxiliary space (codomain) compared to level 3 abstract shapes, it doesn't suffer from major changes in representation if small stem-loops are added or removed. It also doesn't map structures of widely varying stem length to the same element in the codomain.

%example of the conversion process 
\noindent The mapping from a Vienna dot bracket representation can be made simply by removing the dots (unpaired nucleotides) and then creating a tree from it. Since all nodes represent base pairs, the resulting tree can be considered unlabelled. Note that an artificial root is added to all trees. 

\paragraph{Unit Tree Indel Distance}
% relation to the tree edit distance (special scoring scheme)
The unit tree indel distance is a simple and intuitive distance function based on the well known tree edit distance \cite{tai, zhang_shasha}, where insertions and deletions have a unit cost. Since it is applied to base pair trees which are unlabelled, node relabelling comes at no cost.

% interpretation of a score
\noindent Using this scoring scheme, a distance of $n$ means that the smallest number of base pair breakage or formation to transition from a base pair tree to another is $n$. The distance function retains the metric properties.



\subsection*{Optimization goal}

The idea of minimizing the sum of pairwise distance between sets of objects if far from new and a significant body of work has been done in the field of location theory \cite{compact_location}.



\subsection*{MC-Cons}

MC-Cons is a two-step optimization process relying on the minimization of the sum of pairwise distance of two distance functions. First, the 
