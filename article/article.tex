...
\subsection*{Heuristic Consensus Solvers}
...
The genetic algorithm solver uses generic mutation (random substitution) and crossover operators (uniform). To increase convergence rate, an improvement operator using a steepest descent procedure is used.
...
\subsection*{MC-Cons}
% specify that this is an heuristic

MC-Cons is a two-step optimization procedure relying on the minimization of the sum of pairwise distance of two distance functions to create a structural assignment. 

% minimizing the tree distance
\noindent First, \textit{tree consensus} are calculated based on the base pair tree representation of the secondary structures. The intuition is that the topology of the tree is the single most important aspect of structural comparison. The unit tree indel distance 

% within tree consensus, minimizing a naive distance: string edit distance
\noindent For each established tree consensus, all secondary structures whose base pair tree representation is in the tree consensus is filtered and a string edit distance consensus is calculated.
