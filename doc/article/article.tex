...
Given $k$ sets of $ m_1, m_2... m_n$ objects, the consensus problem is to find a set of $k$ objects, one per set, minimizing the sum of their pairwise distance. This is modelled as finding the smallest cycle in a complete $k$-partite graph.
...
\noindent The consensus problem is reformulated as the problem of finding a minimal $k$-clique in a complete weighted $k$-partite graph where $k$ is a free parameter. This problem is known to be intractable
...
Most RNA secondary structure prediction tools allow for the exploration of suboptimal secondary structures. This is important because RNA achieve function only with multiple alternating structures \cite{need_subopts}. Also, the experimentally determined structures of reference structures are usually close to the best structure found.
